...
function SkillsSection() {
  ...
  return (
    <motion.section {...fadeUp} id="skills" className="py-16 px-6 bg-deepMint-400 dark:bg-deepMint-800 text-deepMint-100 dark:text-deepMint-50">
      <h2 className="text-4xl font-bold text-center mb-12">My Experience</h2>

      <motion.div {...zoomIn} className="grid md:grid-cols-2 dark:text-deepMint-50 gap-10 max-w-6xl mx-auto">
        {/* Tabs */}
        <motion.div {...fadeLeft} className="space-y-4">
          {Object.entries(iconMap).map(([key, Icon]) => (
            <motion.div
              ...
              className={`flex items-center justify-between p-4 rounded-lg bg-deepMint-100 dark:bg-deepMint-700   cursor-pointer shadow-md transition duration-300 ${activeTab === key
                ? "bg-deepMint-500 dark:bg-deepMint-500" : "hover:bg-deepMint-50 dark:hover:bg-deepMint-600"
                }`}
            >
              <div className="flex items-center text-deepMint-800 dark:text-deepMint-50  gap-4">
