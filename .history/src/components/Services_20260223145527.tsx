...
const Services = () => {
  ...
  // ألوان خلفية لكل خدمة
  const bgColors = [
    'bg-blue-100 dark:bg-blue-900',     // Web Development
    'bg-purple-100 dark:bg-purple-900', // API Integration
    'bg-pink-100 dark:bg-pink-900',     // UI/UX Design
    'bg-green-100 dark:bg-green-900',   // CMS Solutions
    'bg-yellow-100 dark:bg-yellow-900', // Performance Optimization
    'bg-teal-100 dark:bg-teal-900',     // Consultation & Support
  ];
          ...
          {Object.values(t.services.list).map((service: any, idx) => {
            ...
            return (
              ...
              >
                <motion.div
                  variants={scaleIn}
                  className={`${bgColors[idx]} w-14 h-14 rounded-lg flex items-center justify-center mb-4`}
                >
                  <Icon size={28} className={iconColors[idx]} />
                </motion.div>
                <motion.h3
                  variants={fadeInUp}
