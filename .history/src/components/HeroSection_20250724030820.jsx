...
function HeroSection() {
  ...
  return (
    <section
      id="home"
      className="relative py-20 px-6 bg-white dark:bg-deepMint-800 text-deepMint-800 dark:text-deepMint-50 overflow-hidden"
    >
      {/* 🌊 */}
      ...
      <div className="mt-20 grid md:grid-cols-3 gap-6 text-left">
        {/* {[
          ...
          <motion.div
            ...
            whileHover={{ scale: 1.05, backgroundColor: "#e599c825" }}
            viewport={{ once: true }}
            transition={{ delay: i * 0.1 }}
            className="flex items-start gap-4 bg-deepMint-100 dark:bg-deepMint-900 p-5 rounded-md shadow-md cursor-pointer transition"
          >
            <div
              className={`w-12 h-12 flex items-center justify-center rounded-full ${item.bg} ${item.text} shadow-sm`}
            >
              <i className={`uil ${item.icon} text-xl`} />
            </div>
            <div>
              <h4 className="font-semibold mb-1">{item.title}</h4>
              <span className="text-sm text-deepMint-600 dark:text-deepMint-100">
                {item.subtitle}
              </span>
            </div>
          </motion.div>
        ...
        {
          icons.map((item, i) => (
            <motion.div key={i} {...zoomIn} className="flex items-start gap-4 p-5 rounded-md shadow-md cursor-pointer bg-deepMint-100 dark:bg-deepMint-900">
