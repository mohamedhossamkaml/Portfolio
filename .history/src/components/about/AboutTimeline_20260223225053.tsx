...
const AboutTimeline: React.FC = () => {
  ...
  const TimelineSection = ({ title, items }: { title: string; items: { year: string; event: string }[] }) => (
    <div className="relative mb-24">
      ...
      >
        <h3 className="text-3xl md:text-4xl font-extrabold text-white text-center mb-12">{title}</h3>
        {items.map((milestone, index) => (
          ...
          >
            ...
            <div className={`w-full md:w-5/12 ${index % 2 === 0 ? 'md:text-right' : 'md:text-left'}`}>
              <div className="bg-white dark:bg-gray-800 p-10 rounded-2xl shadow-lg transition-all duration-500 hover:shadow-2xl hover:-translate-y-2 relative overflow-hidden">
                {/* Decorative overlay */}
                ...
                <h4 className="text-2xl md:text-3xl font-bold text-teal-600 dark:text-teal-400 mb-4 transition-colors duration-500 group-hover:text-teal-700 dark:group-hover:text-teal-300">
                  {milestone.year}
                </h4>
                <p className="text-gray-700 dark:text-gray-300 text-lg md:text-xl leading-relaxed transition-opacity duration-500 group-hover:opacity-100">
                  {milestone.event}
                </p>
              </div>
            </div>
            ...
            <div className="w-full md:w-2/12 flex justify-center">
              <div className="w-8 h-8 bg-white dark:bg-teal-400 rounded-full border-4 border-teal-300 dark:border-teal-600 transition-transform duration-500 group-hover:scale-125 group-hover:rotate-6 shadow-md" />
            </div>
  ...
  return (
    <section className="py-28 bg-gradient-to-br from-teal-600 via-teal-700 to-teal-800 dark:from-gray-800 dark:via-gray-850 dark:to-gray-900">
