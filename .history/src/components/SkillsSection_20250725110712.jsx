...
function SkillsSection() {
  ...
  return (
    <motion.section
      {...fadeUp}
      id="skills"
      className="py-16 px-6 bg-baseLight dark:bg-baseDark text-textLight dark:text-textDark"
    >
          ...
          {Object.entries(iconMap).map(([key, Icon]) => (
            <motion.div
              ...
              className={`flex items-center justify-between p-4 rounded-lg cursor-pointer shadow-md transition duration-300 ${activeTab === key
                ? "bg-buttonDark text-baseLight dark:bg-accentDark"
                : "bg-white hover:bg-gray-100 dark:bg-cardDark dark:hover:bg-headerDark"
                }`}
            >
        ...
        <motion.div {...fadeRight} className="space-y-6">
          {skillsData[activeTab].map(({ name, percentage }, i) => (
            ...
            >
              ...
              <div className="w-full h-2 bg-gray-200 dark:bg-softDark rounded-full overflow-hidden">
                <motion.div
                  initial={{ width: 0 }}
                  animate={{ width: `${percentage}%` }}
                  transition={{ duration: 1 }}
                  className="h-2 bg-buttonDark rounded-full"
                />
              </div>
            </motion.div>
          ))}
