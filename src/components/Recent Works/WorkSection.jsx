...
function WorkSection() {
  ...
  return (
    <motion.section {...fadeUp} {...zoomIn} id="work" className="py-16 px-6 bg-deepMint-50 dark:bg-deepMint-900 text-deepMint-800 dark:text-deepMint-50">
      <h2 className="text-4xl font-bold text-center mb-10">Recent Works</h2>

      <motion.div {...fadeLeft} className="flex justify-center gap-4 mb-8 flex-wrap">
        {filters.map(f => (
          <button
            key={f}
      ...
      <motion.div {...fadeRight} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {filtered.map((work, i) => (
